...
grep 'Maaiveldmodel (DTM) ½m'   file_list_ahn5.json | sort -u | cut -d '"' -f4 | sed '/^$/d' > file-list-AHN5-DTM-50cm.txt
/bin/rm file_list_ahn5.json

# Get via OGC API Features AHN6
curl ${AHN6_ITEMS} | jq > file_list_ahn6.json
grep 'Maaiveldmodel (DTM) ½m'   file_list_ahn6.json | sort -u | cut -d '"' -f4 | sed '/^$/d' > file-list-AHN6-DTM-50cm.txt
