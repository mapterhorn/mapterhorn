...
def distribute_tasks(item_filepaths):
    ...
    while item_index < len(item_filepaths):
        ...
        task_requests = glob(utils.store_dir('task-store') + '/*.request')
...
def manage_aggregation():
    ...
    item_filepaths = [filepath.replace('.todo', '') for filepath in glob(f'{utils.store_dir("aggregation-store")}/{aggregation_id}/*-aggregation.csv.todo')]
