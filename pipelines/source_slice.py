"""Slice very large rasters into smaller tiles of a fixed size."""
...
def make_tile(filepath, x, y, w, h):
    """Write a tile window if it contains data (not entirely nodata)."""
...
def slice_tif(filepath, tile_size):
    """Split a GeoTIFF into tiles of at most tile_size pixels."""
    width = None
    height = None
...
def main():
    """Slice every GeoTIFF in a source folder then delete originals."""
    source = None
    tile_size = None
    if len(sys.argv) == 3:
        source = sys.argv[1]
