"""Set nodata values across source GeoTIFFs."""
...
def set_nodata(filepath, nodata):
    """Rewrite raster with a new nodata value using gdal_translate."""
...
def main():
    """Apply nodata to files lacking it (or force) for a given source."""
    source = None
    nodata = None
    force = False
    if len(sys.argv) > 2:
