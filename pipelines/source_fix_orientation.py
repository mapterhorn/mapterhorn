"""Rewrite GeoTIFFs with consistent orientation using gdalwarp."""
...
def fix_orientation(filepath):
    """Convert a GeoTIFF in place to COG with standardized orientation."""
...
def main():
    """Run orientation fixing across all .tif files for a source."""
    source = None
    if len(sys.argv) == 2:
        source = sys.argv[1]
