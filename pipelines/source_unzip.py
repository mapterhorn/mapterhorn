"""Unpack archives for a source and convert contained rasters to COGs."""
...
def unzip(filepath, source):
    """Extract a zip archive into a temp folder and remove the archive."""
...
def un7z(filepath, source):
    """Extract a 7z archive (single or split) then remove the parts."""
    filename = filepath.split("/")[-1]
    utils.run_command(
...
def translate_image(filepath_in, filepath_out, j, total):
    """Convert extracted raster to COG while reporting progress."""
    if j % 1000 == 0:
        print(f"{j} / {total}")
    utils.run_command(
...
def translate_images(filepath, source, suffix):
    """Convert all extracted files with a suffix into output GeoTIFFs."""
...
def is_7z_head_file(filepath):
    """Return True if path is a standalone 7z or first segment of split archive."""
    return filepath.endswith(".7z") or filepath.endswith(".7z.001")


def main():
    """Unzip/un7z all archives for a source and convert contained rasters."""
    source = None
    if len(sys.argv) > 1:
        source = sys.argv[1]
