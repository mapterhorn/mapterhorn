"""Mirror primary downloads to secondary object storage via multipart upload."""
...
def get_file_size(url):
    """Return byte size of remote object, handling gzip-encoded HEADs."""
    r = requests.head(url)
...
def download_range(url, start, end, filepath):
    """Download a byte range of the remote file to a local temp path."""
...
def upload_part(bucket, key, part_number, filepath, upload_id, region, endpoint):
    """Upload a single part file and return its ETag."""
    """
    Requires the following env variables:
    $ export AWS_ACCESS_KEY_ID=MY_KEY
    $ export AWS_SECRET_ACCESS_KEY=MY_SECRET
...
def complete_multipart_upload(bucket, key, upload_id, parts, region, endpoint):
    """Finalize multipart upload given part metadata."""
    """
    Requires the following env variables:
    $ export AWS_ACCESS_KEY_ID=MY_KEY
    $ export AWS_SECRET_ACCESS_KEY=MY_SECRET
...
def process_range(
    ...
    """Download a byte range and upload it as one multipart piece with retries."""
    retries = 0
    max_retries = 3
    while True:
        try:
...
def mirror_http_resource_to_s3(url, bucket, key, region, endpoint, filename):
    """Mirror a single file by downloading in parts and uploading to S3-compatible storage."""
...
def get_filenames(mirror_base_url):
    """Determine which filenames need mirroring based on md5 comparisons."""
    filenames = []
...
def main():
    """Entry point to mirror missing or outdated bundles to configured bucket."""
