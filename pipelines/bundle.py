...
def get_parent_to_filepaths(only_dirty, num_aggregations):
    filepaths = sorted(glob(utils.store_dir('pmtiles-store') + '/*.pmtiles') + glob(utils.store_dir('pmtiles-store') + '/*/*.pmtiles'))
...
def create_archive(filepaths, name):
    utils.create_folder(utils.store_dir('bundle-store'))
    out_filepath = f'{utils.store_dir("bundle-store")}/{name}.pmtiles'
    checksum = None
    min_z = math.inf
    max_z = 0
    min_lon = math.inf
    ...
    utils.create_folder(utils.store_dir('meta-store') + '/bundle')
    filesize = os.path.getsize(out_filepath)
    with open(f'{utils.store_dir("meta-store")}/bundle/{name}.json', 'w') as f:
        json.dump({
            'size': filesize,
            'md5sum': checksum,
            'min_lon': min_lon,
