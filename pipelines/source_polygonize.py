"""Polygonize source rasters to coverage polygons and merge them."""
...
def polygonize_tif(source, filename):
    """Convert a raster to a coverage mask and polygonize into GPKG."""
    mask_filepath = f"polygon-store/{source}/{filename}"
    utils.run_command(
        f'GDAL_CACHEMAX=1024 gdal_calc.py -A source-store/{source}/{filename} --outfile={mask_filepath} --calc="A*0+1" --type=Byte --overwrite',
        silent=SILENT,
    )
    utils.run_command(
        f'GDAL_CACHEMAX=1024 gdal_polygonize.py {mask_filepath} -b 1 -f "GPKG" polygon-store/{source}/{filename}.gpkg -overwrite',
        silent=SILENT,
    )
...
def get_filenames(source):
    """Read filenames from bounds.csv for a source."""
    lines = None
...
def polygonize_source(source, processes):
    """Polygonize all tiles for a source in parallel."""
...
def merge_source(source):
    """Merge individual polygons into a union coverage geopackage."""
    filenames = get_filenames(source)
    merged_filepath = f"polygon-store/{source}/merged.gpkg"
    if os.path.isfile(merged_filepath):
        os.remove(merged_filepath)
    command = (
        f"ogr2ogr -f GPKG {merged_filepath} polygon-store/{source}/{filenames[0]}.gpkg"
    )
    ...
    utils.run_command(
        f'ogr2ogr -f GPKG {union_filepath} {merged_filepath} -nln union -dialect sqlite -sql "SELECT ST_Union(ST_MakeValid(geom)) AS geom FROM out"',
        silent=False,
    )


def main():
    """Polygonize a source and merge polygons to polygon-store/{source}.gpkg."""
    source = None
    processes = None
    if len(sys.argv) == 3:
        source = sys.argv[1]
        ...
        print(
            "Not enough arguments. Usage: source_polygonize.py {{source}} {{processes}}"
        )
        exit()
